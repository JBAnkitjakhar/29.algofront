...
import type { CreateSolutionRequest } from "@/having/adminSolutions/types";
import toast from "react-hot-toast";
import { QuestionSelector } from "@/having/adminSolutions/components/QuestionSelector";
...
export default function CreateSolutionPage() {
  ...
  const { data: questionsData } = useQuestionsMetadata();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    createSolutionMutation.mutate(
      { questionId: selectedQuestionId, request: updatedFormData },
      {
        onSuccess: () => {
      ...
      <div className="flex-1 flex flex-col max-w-5xl mx-auto px-6 py-8 overflow-y-auto">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-gray-400">
            Add an official solution with rich content, code templates, and visualizers
          </p>
        </div>
        ...
        <div className="sticky top-0 p-4 space-y-4 max-h-screen overflow-y-auto">
          <div className="space-y-2">
            <button
              ...
              disabled={createSolutionMutation.isPending || isOverLimit}
